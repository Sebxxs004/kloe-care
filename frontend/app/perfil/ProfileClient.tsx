...
/* removed SPECIES_EMOJI - no longer used */
          ...
          <div className="pf-section animate-up" style={{ animationDelay: '0.14s' }}>
            ...
            >
              ...
              <span className="pf-section-label">
                {currentPet ? currentPet.name : 'Mis mascotas'}
              </span>
              <svg className="pf-chevron" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M6 9l6 6 6-6"/>
              </svg>
            ...
            {openSection === 'mascota' && currentPet && (
              <div className="pf-section-body animate-up">
                ...
                <PetNameForm    key={`name-${currentPet.id}`}   pet={currentPet} onSuccess={showToast} onError={m => showToast(m, 'err')} />
                <div className="pf-divider" />
                <PetBirthForm   key={`birth-${currentPet.id}`}  pet={currentPet} onSuccess={showToast} onError={m => showToast(m, 'err')} />
                <div className="pf-divider" />
                <PetGenderForm  key={`gender-${currentPet.id}`} pet={currentPet} onSuccess={showToast} onError={m => showToast(m, 'err')} />
              </div>
            )}
          </div>
        )}
